#' @title MPS estimation of the left-truncated Pareto distribution
#' @description Apply MPS estimation to the left-truncated  Pareto distribution.
...
pareto_LTMPS = function(data,init_value,u){
  ...
  opt_pareto <- function(par){
    ...
    est = -sum(log(c(1, ppareto(dat,shape,scale))-
                     ppareto(c(dat,u),shape,scale)))+
      (n+1)*log(1-ppareto(u,shape,scale))
    return(est)
  }
