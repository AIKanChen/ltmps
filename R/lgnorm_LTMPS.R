#' @title   MPS estimation of the left truncated Log-normal distribution
#' @description Apply the left-truncated MPS estimation to the Log-normal distribution.
...
lgnorm_LTMPS = function(data,init_value,u){
  ...
  opt_lgnorm <- function(par){
    ...
    est = -sum(log(c(1, plnorm(dat,mu,sigma))-
                     plnorm(c(dat,u),mu,sigma)))+
      (n+1)*log(1-plnorm(u,mu,sigma))
    return(est)
  }
