#' @title   MLE estimation of the left truncated Burr distribution
#' @description Apply the left-truncated MLE estimation to the Burr distribution.
...
#' @examples
#' data0 = rburr(2000,1,4,0.2)
#' data = data0[data0>5]
#' fit = burr_LTMLE(data = data,init_value = c(1,4,0.2),u=5)
#' fit$par.ests


burr_LTMLE<-function(data,init_value,u){
  ...
  opt_burr <- function(par){
    ...
    est=-sum(log(c*k*(dat/lambda)^(c-1)/lambda/(1+(dat/lambda)^c)^(k+1)))+
      n*log((1+(u/lambda)^c)^(-k))
    return(est)
  }
  ...
  par.ses <- sqrt(diag(varcov))*exp(fit$par)
