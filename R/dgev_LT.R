#' @title  Left-truncated Generalized Extreme Value Distribution
#' @description Cumulative probability, quantiles, density generation from left-tructed GEV distribution.
...
dgev_LT<-function (x,xi,sigma,u){
  if(sigma/xi>u){stop("The left truncated point is less than mu.")}
  if(x<u){stop("The value is less than left truncated point")}
  dgev(x,xi=xi,sigma=sigma,mu=sigma/xi)/(1-pgev(u,xi=xi,sigma=sigma,mu=sigma/xi))
}
